...
export function getMonthIngredients(month, foods, ageGroupId, dietTypeId = 'normal') {
...
export function buildCalendarMonthPlan(year, month, foods, ageGroupId = 'adult', dietTypeId = 'normal') {
  const dedup = getMonthIngredients(month, foods, ageGroupId, dietTypeId)
  ...
  const rawProtein = dedup.filter(f => PROTEIN_CATS.includes(f.category))
  const veg   = priorityFn ? prioritizedShuffle(dedup.filter(f => VEG_CATS.includes(f.category)), priorityFn) : shuffle(dedup.filter(f => VEG_CATS.includes(f.category)))
  const grain = shuffle(dedup.filter(f => GRAIN_CATS.includes(f.category)))
  const fruit = priorityFn ? prioritizedShuffle(dedup.filter(f => FRUIT_CATS.includes(f.category)), priorityFn) : shuffle(dedup.filter(f => FRUIT_CATS.includes(f.category)))
  ...
  const protein = rawProtein.length
    ...
    : shuffle(dedup.filter(f => VEG_CATS.includes(f.category)))
  // 국 재료 풀 — 국물 요리에 어울리는 카테고리만 사용, 부족하면 채소로 폴백
  const rawSoup = dedup.filter(f => SOUP_ELIGIBLE_CATS.includes(f.category))
  const soupPool = shuffle(rawSoup.length ? rawSoup : dedup.filter(f => VEG_CATS.includes(f.category)))
