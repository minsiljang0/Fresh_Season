...
export function getBasicRecipe(ingredient, dish) {
  ...
  const method = getDishMethod(ingredient, dish)
  const steps = (method && RECIPE_STEPS_BY_METHOD[method]) || RECIPE_STEPS_FALLBACK
  return steps.map(s => fillTemplate(s, ingredient))
}

// dish 이름에서 조리법 접미사만 뽑아낸다 (예: "고등어 구이" + "고등어" → "구이").
// 실제 등록된 레시피 중 이 조리법(category)과 일치하는 걸 우선 매칭할 때 사용.
export function getDishMethod(ingredient, dish) {
  if (!dish || dish === ingredient) return null
  return dish.startsWith(ingredient) ? dish.slice(ingredient.length).trim() : null
}
...
function buildStructuredMeal(slots, dayIndex, scaleFactor = 1, stapleFactor = 1) {
  ...
  if (slots.staple) {
    ...
    items.push({ role: 'staple', ingredient: food.ingredient, ingredientId: food.id, dish: `${food.ingredient} 밥`, kcal: round10(kcalFor(food.category) * stapleFactor * scaleFactor) })
  }
  if (slots.soup) {
    const { food } = slots.soup
    // 국물 요리는 건더기보다 국물 자체의 열량 비중도 있어 기본 국물 베이스를 더 반영
    items.push({ role: 'soup', ingredient: food.ingredient, ingredientId: food.id, dish: soupDishFor(food.ingredient, food.category), kcal: round10((80 + kcalFor(food.category) * 0.8) * scaleFactor) })
  }
  if (slots.mainDish) {
    const { food, dishIdx = 0 } = slots.mainDish
    items.push({ role: 'mainDish', ingredient: food.ingredient, ingredientId: food.id, dish: getDishSuggestions(food.ingredient, food.category)[dishIdx % 3], kcal: round10(kcalFor(food.category) * scaleFactor) })
  }
  ...
  sideDishes.filter(Boolean).forEach(({ food, dishIdx = 0 }) => {
    items.push({ role: 'sideDish', ingredient: food.ingredient, ingredientId: food.id, dish: getDishSuggestions(food.ingredient, food.category)[dishIdx % 3], kcal: round10(kcalFor(food.category) * scaleFactor) })
  })
