...
function buildMeal(items) {
  ...
  const dishes = valid.map(({ food, dishIdx }) => ({
    ...
    kcal: kcalFor(food.category),
  }))
  const kcal = 300 /* 기본 밥 한공기 */ + dishes.reduce((s, d) => s + d.kcal, 0)
  return { dishes, kcal }
}
...
export function buildCalendarMonthPlan(year, month, foods, ageGroupId = 'adult') {
  ...
  const days = Array.from({ length: daysInMonth }, (_, i) => {
    ...
    const breakfast = buildMeal([g && { food: g, dishIdx: 0 }, fr && { food: fr, dishIdx: i }])
    const lunch     = buildMeal([g && { food: g, dishIdx: 1 }, v1 && { food: v1, dishIdx: i }, p1 && { food: p1, dishIdx: i }])
    const dinner    = buildMeal([g && { food: g, dishIdx: 2 }, v2 && { food: v2, dishIdx: i + 1 }, p2 && { food: p2, dishIdx: i + 1 }])
