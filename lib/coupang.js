...
export function resolveCoupangDisplay(links, widgets, { coupang_url, coupang_banner_html } = {}) {
  ...
  // 위젯(사진 카드)이 있으면 위젯만, 없으면 링크 버튼만 — 둘 다 있어도 중복 노출하지 않는다.
  if (coupang_banner_html) {
    return { links: [], widgets: [coupang_banner_html] }
  }
  if (coupang_url) {
    return { links: [{ label: '쿠팡에서 구매하기', url: coupang_url }], widgets: [] }
  }
