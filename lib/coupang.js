...
// 관리자 설정(경로 / 내 번호 / 위젯)을 활용해 대체 링크·위젯을 만들어준다.

export function buildCoupangFallbackUrl(coupangSettings, query) {
  if (!coupangSettings) return ''
  const { searchTemplate, partnerId, partnerPath } = coupangSettings

  if (searchTemplate && query) {
    return searchTemplate
      .replace('{query}', encodeURIComponent(query))
      .replace('{channel}', encodeURIComponent(partnerId || ''))
  }
...
export function resolveCoupangDisplay(coupangSettings, { coupang_url, coupang_banner_html } = {}, query) {
  ...
  if (!coupangSettings || !coupangSettings.fallbackEnabled) {
    return { url: '', widgetHtml: '', isFallback: false }
  }
  const mode = coupangSettings.fallbackMode || 'link'
  const url = mode !== 'widget' ? buildCoupangFallbackUrl(coupangSettings, query) : ''
  const widgetHtml = mode !== 'link' ? (coupangSettings.widgetHtml || '') : ''
