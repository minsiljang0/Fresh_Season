 ...
 *  images 배열은 스텝과의 위치(index)를 유지하기 위해 빈 문자열도 그대로 둔다. */
export function extractStepImages(content) {
  ...
  try {
    ...
    if (Array.isArray(parsed)) images = parsed.map(x => (typeof x === 'string' ? x : ''))
  } catch {}
  return { content: content.slice(0, m.index), images }
}

/** 순수 마크다운 본문 + 이미지 배열을 합쳐서 저장용 content 문자열로 만든다.
 *  images의 각 자리는 해당 순번 스텝과 매칭되므로 빈 문자열도 위치 보존을 위해 그대로 저장한다. */
export function injectStepImages(content, images) {
  const clean = (content || '').replace(MARKER_RE, '') // 기존 마커 제거 후 새로 붙임
  const safeImages = Array.isArray(images) ? images.map(s => (s || '').trim()) : []
  if (!safeImages.some(Boolean)) return clean
