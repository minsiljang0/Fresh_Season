...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (!category || !angle || !title || !slug) return res.status(400).json({ error: 'category, angle, title, slug는 필수입니다' })
    const row = {
      id: genId(), category, angle, title, slug,
      memo: memo || null,
      target_keyword: targetKeyword || null,
      search_pc: searchPc != null ? Number(searchPc) : null,
      search_mobile: searchMobile != null ? Number(searchMobile) : null,
      search_total: searchTotal != null ? Number(searchTotal) : null,
