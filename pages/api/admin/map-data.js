    ...
    try {
      ...
      if (type === 'ingredients') {
        const { data, error } = await supabase.from('ingredients')
          .insert([{ id: genId(), name: body.name, category: body.category || 'veg', description: body.description || '', coupang_url: body.coupang_url || '', caution: body.caution || '', is_special: body.is_special || false, is_limited: body.is_limited || false, limited_start: body.limited_start || null, limited_end: body.limited_end || null }])
          .select().single()
        if (error) throw error
        return res.status(200).json(data)
      }
