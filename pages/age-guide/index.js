      ...
      <main className="wrap" style={{ paddingTop: 20, paddingBottom: 60 }}>
        <section style={{ marginBottom: 24 }}>
          ...
          <p style={{ fontSize: 13, color: 'var(--text2)', lineHeight: 1.6 }}>
            공신력 있는 공공기관 자료를 바탕으로 연령대별로 <b>챙기면 좋은 영양소</b>와 <b>관심 가져야 할 질환·검진 이슈</b>를 정리했어요.
        ...
        <div className="grid-auto" style={{ marginBottom: 32 }}>
          {AGE_GUIDE_ORDER.map(id => {
            ...
            return (
              <Link key={id} href={`/age-guide/${id}`} className="card">
                <div style={{ fontSize: 32, marginBottom: 6 }}>{g.emoji}</div>
                <div style={{ fontSize: 18, fontWeight: 900, marginBottom: 2 }}>{g.label}</div>
                <div style={{ fontSize: 13, color: 'var(--text2)', marginBottom: 8 }}>{g.range}</div>
                <p style={{ fontSize: 12, color: 'var(--text3)', lineHeight: 1.5 }}>{g.nutrients[0]}</p>
              </Link>
            )
          })}
        </div>
