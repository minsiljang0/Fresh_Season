...
export default function BlogPost({ post, html, allPosts, stepImages, customCategories }) {
  ...
  // 관리자 전용 — 로그인 상태에서만 이 글의 제목 점수·SEO 점수를 가져와 보여준다 (일반 방문자에게는 노출되지 않음)
  const [adminScores, setAdminScores] = useState(null)
  useEffect(() => {
    if (!post?.slug) return
    let adminToken = ''
    ...
    fetch(`/api/blog/posts?slug=${post.slug}`, { headers: { 'x-admin-token': adminToken } })
      ...
      .then(data => {
        if (data && (data.title_score != null || data.seo_score != null)) {
          setAdminScores({ title_score: data.title_score, seo_score: data.seo_score })
        }
      })
      .catch(() => {})
  }, [post?.slug])
      ...
      <main className="wrap" style={{ maxWidth: 780 }}>
        ...
        {!post ? (
          ...
          <article style={{ padding: '40px 0 64px' }}>
            ...
            {/* 관리자 전용 — 제목 점수·SEO 점수 (로그인 상태에서만 보임, 일반 방문자에게는 표시 안 됨) */}
            {adminScores && (
              <div style={{ display: 'inline-flex', gap: 10, alignItems: 'center', marginBottom: 20, padding: '8px 14px', background: '#fef3c7', border: '1px dashed #f59e0b', borderRadius: 8, fontSize: 12, fontWeight: 700, color: '#92400e' }}>
                <span>🔒 관리자 전용</span>
                {adminScores.title_score != null && <span>제목 점수: {adminScores.title_score}/10</span>}
                {adminScores.seo_score != null && <span>SEO 점수: {adminScores.seo_score}/100</span>}
              </div>
            )}
