...
const CATEGORY_OPTIONS = [
  ...
  { value: 'season',     label: '제철/계절' },
  { value: 'health',     label: '건강·효능' },
  { value: 'tips',       label: '요리 팁' },
  { value: 'region',     label: '지역 특산' },
          ...
          {form.type === 'keyword' && (
            <div>
              ...
              <input value={form.keyword} onChange={e => set('keyword', e.target.value)}
                style={{ ...S.input }} placeholder="예: 봄나물 효능" />
            </div>
          )}
          <div>
...
export default function ContentIdeaPanel({ adminToken }) {
  ...
  const addIdea = async (form) => {
    setShowAdd(false)
    // category를 tool_id 자리에 저장 (API 호환성 유지)
    const payload = { ...form, tool_id: form.category || null }
  ...
  const visibleIdeas = ideas.filter(i => {
    ...
    if (filterCategory && i.tool_id !== filterCategory) return false
        ...
        {tabs.map(tab => {
          ...
          return (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{
              ...
              <span>{tab.label}</span>
              {cnt > 0 && (
                <span style={{ fontSize: 11, background: '#22c55e22', color: ACCENT, padding: '1px 6px', borderRadius: 10, fontWeight: 700 }}>
                  {cnt}
                </span>
              )}
            </button>
