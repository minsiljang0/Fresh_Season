  ...
  return (
    <div>
      <div style={{ marginBottom: 16, fontSize: 13, color: '#71717a', lineHeight: 1.7 }}>
        ...
        Claude가 황금키워드를 발견했을 때 기존 시도 페이지에 기능으로 추가하면 좋겠다고 판단하면 여기에 쌓입니다.
      </div>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {idea.keyword && (
                      <div style={{ fontSize: 12, color: '#71717a', marginBottom: 6 }}>
                        ...
                        {idea.total ? <span style={{ marginLeft: 8 }}>검색수 <strong style={{ color: '#22c55e' }}>{fmt(idea.total)}</strong></span> : ''}
          ...
          {allRows.map(row => {
            ...
            return (
              <div key={row.hint} style={{ marginBottom: 8 }}>
                <div style={{
                  background: '#1c1c1e',
                  border: `1px solid ${isExpanded ? '#22c55e' : needsUpdate ? '#166534' : '#2a2a2a'}`,
                      ...
                      {row.count > 0 && (
                        <div style={{ fontSize: 12, color: '#52525b', marginTop: 2 }}>
                          ...
                          {row.null_doc_count > 0
                            ? <span style={{ color: '#4ade80', marginLeft: 6 }}>· 문서수 미수집 <b>{fmt(row.null_doc_count)}개</b> 남음</span>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <button onClick={e => { e.stopPropagation(); handleUpdateByHint(row.hint) }} disabled={isLoading} style={{
                      background: (needsUpdate || (row.null_doc_count || 0) > 0) ? '#22c55e' : '#27272a',
