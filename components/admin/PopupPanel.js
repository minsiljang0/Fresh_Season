  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:9000, display:'flex', alignItems:'center', justifyContent:'center' }}>
      <div style={{ background:'#fff', border:'1px solid #d1e8d1', borderRadius:14, padding:28, width:500, maxHeight:'90vh', overflowY:'auto', boxShadow:'0 20px 60px rgba(0,0,0,0.15)' }}>
        <div style={{ fontSize:16, fontWeight:700, marginBottom:20, color:'#0f1f0f' }}>📢 팝업 추가</div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                <input type="color" value={form.bg_color} onChange={e => set('bg_color', e.target.value)}
                  style={{ width:40, height:36, border:'1px solid #d1e8d1', borderRadius:6, cursor:'pointer', padding:2 }} />
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <label htmlFor="is_active" style={{ fontSize:14, color:'#0f1f0f', cursor:'pointer' }}>즉시 활성화</label>
          </div>
          ...
          <div>
            ...
            <div style={{
              ...
              maxWidth:360, margin:'0 auto', boxShadow:'0 4px 20px rgba(0,0,0,0.15)',
            }}>
  ...
  return (
    <div>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20 }}>
        <div>
          <div style={{ fontSize:17, fontWeight:700, color:'#0f1f0f' }}>📢 팝업 관리</div>
          <div style={{ fontSize:12, color:'#888', marginTop:3 }}>
            활성 팝업 <span style={{ color:ACCENT, fontWeight:700 }}>{activeCount}개</span> / 전체 {popups.length}개
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          {popups.map(p => (
            <div key={p.id} style={{
              background:'#fff', border:`1px solid ${p.is_active ? '#bbf7d0' : '#e5e7eb'}`,
              borderLeft:`4px solid ${p.is_active ? ACCENT : '#d1d5db'}`,
              ...
              <div style={{ display:'flex', alignItems:'flex-start', gap:12 }}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4 }}>
                    <span style={{ fontWeight:700, fontSize:14, color:'#0f1f0f' }}>{p.title}</span>
                    <span style={{
                      fontSize:11, fontWeight:700, padding:'2px 8px', borderRadius:10,
                      background: p.is_active ? '#dcfce7' : '#f3f4f6',
                      color: p.is_active ? ACCENT : '#9ca3af',
                    }}>{p.is_active ? '활성' : '비활성'}</span>
                    {p.expires_at && (
                      <span style={{ fontSize:11, color:'#f59e0b' }}>~{fmtDate(p.expires_at)}</span>
                    )}
                  </div>
                  <div style={{ fontSize:13, color:'#6b7280', lineHeight:1.5, marginBottom:4 }}>{p.content}</div>
                  {p.link_url && (
                    <div style={{ fontSize:12, color:'#16a34a' }}>🔗 {p.link_url}</div>
                  )}
                  <div style={{ fontSize:11, color:'#bbb', marginTop:4 }}>등록 {fmtDate(p.created_at)}</div>
                </div>

                <div style={{ display:'flex', gap:6, flexShrink:0 }}>
                  <button onClick={() => toggleActive(p.id, p.is_active)}
                    style={{ background:'none', border:`1px solid ${p.is_active ? '#fca5a5' : '#86efac'}`, borderRadius:7, color: p.is_active ? '#ef4444' : ACCENT, cursor:'pointer', padding:'5px 10px', fontSize:12, fontWeight:700 }}>
