...
export default function LegalPanel({
  ...
  return (
    <div style={S.card}>
      ...
      <p style={{ color: '#4b6e4b', fontSize: 13, marginBottom: 18 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
        <button onClick={() => setDoc('terms')} style={{
          ...S.btn(doc === 'terms' ? '#16a34a' : '#f5f9f5'),
          color: doc === 'terms' ? '#fff' : '#4b6e4b', padding: '8px 16px', fontSize: 13,
        }}>📜 이용약관</button>
        <button onClick={() => setDoc('privacy')} style={{
          ...S.btn(doc === 'privacy' ? '#16a34a' : '#f5f9f5'),
          color: doc === 'privacy' ? '#fff' : '#4b6e4b', padding: '8px 16px', fontSize: 13,
        }}>🔒 개인정보처리방침</button>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <button onClick={() => setLang('ko')} style={{
          ...S.btn(lang === 'ko' ? '#3b82f6' : '#f5f9f5'),
          color: lang === 'ko' ? '#fff' : '#4b6e4b', padding: '6px 14px', fontSize: 12,
        }}>🇰🇷 한국어</button>
        <button onClick={() => setLang('en')} style={{
          ...S.btn(lang === 'en' ? '#3b82f6' : '#f5f9f5'),
          color: lang === 'en' ? '#fff' : '#4b6e4b', padding: '6px 14px', fontSize: 12,
        }}>🇺🇸 English</button>
      </div>
      ...
      {lang === 'en' && !current.value?.trim() && (
        <p style={{ color: '#4b6e4b', fontSize: 12, marginTop: 8 }}>
