...
const STORAGE_KEY = 'freshseason_blog_image_uploads'
...
export default function BlogImagePanel({ adminToken, showToast }) {
  ...
  return (
    <div style={S.card}>
      ...
      <p style={{ fontSize: 13, color: '#4b6e4b', marginBottom: 16, lineHeight: 1.6 }}>
      ...
      <div
        ...
        style={{
          border: '2px dashed #d1e8d1', borderRadius: 12, padding: '32px 20px',
          textAlign: 'center', cursor: 'pointer', marginBottom: 20, background: '#f5f9f5',
        }}
      >
        <div style={{ fontSize: 28, marginBottom: 8 }}>📤</div>
        <div style={{ fontSize: 14, color: '#0f1f0f', fontWeight: 600 }}>클릭하거나 이미지를 여기로 끌어다 놓으세요</div>
        <div style={{ fontSize: 12, color: '#4b6e4b', marginTop: 4 }}>jpg / png / gif / webp, 10MB 이하 (여러 장 동시 선택 가능)</div>
        <input
          ref={fileInputRef}
          type="file"
      ...
      {items.length === 0 && (
        <p style={{ fontSize: 13, color: '#4b6e4b' }}>아직 업로드한 이미지가 없습니다.</p>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {items.map(it => (
          <div key={it.id} style={{ ...S.row, display: 'flex', alignItems: 'center', gap: 14 }}>
            <div style={{
              flexShrink: 0, width: 28, height: 28, borderRadius: '50%', background: '#fff',
              border: '1px solid #d1e8d1', display: 'flex', alignItems: 'center', justifyContent: 'center',
            ...
            <img
              ...
              style={{ width: 56, height: 56, objectFit: 'cover', borderRadius: 8, flexShrink: 0, background: '#e5efe5', cursor: (it.previewUrl || it.url) ? 'zoom-in' : 'default' }}
            />
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13, color: '#0f1f0f', fontWeight: 600, marginBottom: 6, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {it.seq}번 · {it.filename}
              </div>
              {it.status === 'uploading' && <div style={{ fontSize: 12, color: '#4b6e4b' }}>업로드 중...</div>}
              {it.status === 'error' && <div style={{ fontSize: 12, color: '#dc2626' }}>❌ {it.error}</div>}
              {it.status === 'done' && (
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  <input readOnly value={it.url} onFocus={e => e.target.select()} style={{ ...S.input, fontSize: 12, padding: '6px 10px' }} />
                  <button onClick={() => copyUrl(it.id, it.seq, it.url)} style={{ ...S.btn(copiedId === it.id ? '#4b6e4b' : '#16a34a'), flexShrink: 0, padding: '6px 14px', fontSize: 12 }}>
            ...
            <button onClick={() => deleteItem(it)} title="삭제" style={{
              flexShrink: 0, background: 'none', border: '1px solid #d1e8d1', borderRadius: 8,
              color: '#dc2626', fontSize: 12, fontWeight: 700, padding: '8px 12px', cursor: 'pointer', fontFamily: 'inherit',
            }}>삭제</button>
          </div>
        ))}
      </div>
      ...
      {previewItem && (
        <div
          ...
          style={{
            position: 'fixed', inset: 0, background: 'rgba(15,31,15,0.75)', zIndex: 1000,
