...
import { ING_CATEGORIES, ING_GROUPS, ING_CAT_MAP } from '../../lib/ingredientCategories'
...
function IngredientTab({ adminToken, showToast, confirmDelete, allHealths, allTvShows, refreshHealths }) {
  ...
  const IngFormFields = ({ f, setF }) => (
    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
      ...
      <div style={{ gridColumn:'1/-1' }}>
        <label style={S.label}>🛒 쿠팡 URL</label>
        <input value={f.coupang_url||''} onChange={e=>setF(p=>({...p,coupang_url:e.target.value}))} placeholder="https://coupa.ng/... 또는 https://link.coupang.com/a/..." style={S.input} />
      </div>
      <div style={{ gridColumn:'1/-1' }}>
        <label style={S.label}>📺 쿠팡 배너 iframe 코드</label>
...
// ══════════════════════════════════════════════════════════
// RecipeTab — 레시피 등록 (재료·조리도구·조리순서 하위 관리 포함)
// ══════════════════════════════════════════════════════════
function RecipeTab({ adminToken, showToast, confirmDelete, allIngredients, onBlogWrite }) {
  const [formOpen, setFormOpen] = useState(false)
  const EMPTY = { title:'', summary:'', source_url:'' }
  const [list, setList] = useState([])
  const [loading, setLoading] = useState(true)
  const [form, setForm] = useState(EMPTY)
  const [saving, setSaving] = useState(false)
  ...
  const addStep = async (recipeId) => {
    ...
    try {
      await apiFetch(api('recipe_steps'), { method:'POST', headers:{'Content-Type':'application/json','x-admin-token':adminToken},
        body:JSON.stringify({ recipe_id: recipeId, order_num: orderNum, ...stepForm }) })
  ...
  const addIngredient = async (recipeId) => {
    ...
    try {
      await apiFetch(api('recipe_ingredients'), { method:'POST', headers:{'Content-Type':'application/json','x-admin-token':adminToken},
        body:JSON.stringify({ recipe_id: recipeId, ...ingForm }) })
  ...
  const addTool = async (recipeId) => {
    ...
    try {
      await apiFetch(api('recipe_tools'), { method:'POST', headers:{'Content-Type':'application/json','x-admin-token':adminToken},
        body:JSON.stringify({ recipe_id: recipeId, ...toolForm }) })
                ...
                {editId===r.id ? (
                  ...
                  <>
                    <div style={{ display:'flex', justifyContent:'space-between', gap:14 }}>
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ fontWeight:700, color:'#111', marginBottom:3 }}>🍳 {r.title}</div>
                      ...
                      <div style={{ display:'flex', gap:4, flexShrink:0 }}>
                        ...
                        <button onClick={()=>{ setEditId(r.id); setEditForm({title:r.title,summary:r.summary||'',source_url:r.source_url||''}) }}
                          style={{ ...S.btnGhost, padding:'4px 10px', fontSize:12 }}>✏️</button>
...
export default function MapAdminPanel({ adminToken, onBlogWrite }) {
  ...
  return (
    <div>
      ...
      {subTab === 'recipe' && (
        <RecipeTab adminToken={adminToken} showToast={showToast} confirmDelete={confirmDelete}
          allIngredients={allIngredients} onBlogWrite={onBlogWrite} />
      )}
    </div>
  )
}
